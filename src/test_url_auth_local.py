...
async def test_url_auth():
    """测试URL认证功能"""
    print("=== URL认证功能测试（本地版本） ===")
    
    # 创建配置
    config = {
        "config_concurrent": 3,  # 并发数
        "timeout": 2000,  # 缩短超时时间到2秒
    ...
    # 创建测试设备 - 使用本地回环地址
    test_devices = [
        MockDevice("127.0.0.1", 80, "admin", "admin"),
        MockDevice("localhost", 80, "admin", "admin123"),
        MockDevice("127.0.0.2", 80, "admin", "password")
    ]
    
    print(f"测试设备数量: {len(test_devices)}")
    print(f"测试命令数量: {len(config['cgi_commands'])}")
    print(f"并发控制数: {config['config_concurrent']}")
    print("注意：此测试使用本地回环地址，主要用于验证异步功能而非实际设备连接")
    print()
...
def test_single_command():
    ...
    # 创建测试设备
    device = MockDevice("127.0.0.1", 80, "admin", "admin")
